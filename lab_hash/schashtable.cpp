...
void SCHashTable<K, V>::insert(K const& key, V const& value)
{
    ...
    table[curHash].push_front(pair<K, V>(key, value));
    elems++;
    if(shouldResize())
        resizeTable();
}
...
{
    unsigned curHash = hash(key, size);
    if(table[curHash].empty())
        return;
    typename list<pair<K, V>>::iterator it = table[curHash].begin();
    while(it!= table[curHash].end()){
        if((*it).first == key){
            table[curHash].erase(it);
            elems--;
            break;
        }
        it++;
